...
document.addEventListener("click", (e) => {
  if (e.target.id === "guessBtn") {
    ...
    socket.emit("guessNumber", { numero, username });
...
function updateVisual() {
  ...
  myDiv.style.backgroundImage = `url("${getSpritePath(currentDirection)}")`;
}
...
socket.on("playersUpdate", (players) => {
  for (const id in players) {
    ...
    div.style.backgroundImage = `url("${getSpritePath(p.direction || "down")}")`;
  }
